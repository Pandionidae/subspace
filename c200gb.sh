...
function eof_docker_compose {
  mkdir -p $HOME/bundlr/validator-rust/docs/doc/
  sudo tee <<EOF >/dev/null $HOME/bundlr/validator-rust/docs/doc/docker-compose.yml
  version: "3.7"
  services:
    node:
...
function docker_compose_up {
  docker-compose -f $HOME/bundlr/validator-rust/docs/doc/docker-compose.yml up -d
}

function echo_info {
  echo -e "${GREEN}Для остановки ноды и фармера subspace: ${NORMAL}"
  echo -e "${RED}   docker-compose -f $HOME/bundlr/validator-rust/docs/doc/docker-compose.yml down \n ${NORMAL}"
  echo -e "${GREEN}Для запуска ноды и фармера subspace: ${NORMAL}"
  echo -e "${RED}   docker-compose -f $HOME/bundlr/validator-rust/docs/doc/docker-compose.yml up -d \n ${NORMAL}"
  echo -e "${GREEN}Для перезагрузки ноды subspace: ${NORMAL}"
  echo -e "${RED}   docker-compose -f $HOME/bundlr/validator-rust/docs/doc/docker-compose.yml restart node \n ${NORMAL}"
  echo -e "${GREEN}Для перезагрузки фармера subspace: ${NORMAL}"
  echo -e "${RED}   docker-compose -f $HOME/bundlr/validator-rust/docs/doc/docker-compose.yml restart farmer \n ${NORMAL}"
  echo -e "${GREEN}Для проверки логов ноды выполняем команду: ${NORMAL}"
  echo -e "${RED}   docker-compose -f $HOME/bundlr/validator-rust/docs/doc/docker-compose.yml logs -f --tail=100 node \n ${NORMAL}"
  echo -e "${GREEN}Для проверки логов фармера выполняем команду: ${NORMAL}"
  echo -e "${RED}   docker-compose -f $HOME/bundlr/validator-rust/docs/doc/docker-compose.yml logs -f --tail=100 farmer \n ${NORMAL}"
}

function delete_old {
  docker-compose -f $HOME/bundlr/validator-rust/docs/doc/docker-compose.yml down -v &>/dev/null
